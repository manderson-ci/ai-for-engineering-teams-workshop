  ...
  return (
    <div
      ...
      className={[
        ...
        'flex flex-col gap-2',
        onClick
          ? 'cursor-pointer transition-shadow hover:shadow-md hover:border-gray-300'
          : '',
      ]
        .join(' ')
        .trim()}
    >
      ...
      <div className="flex items-center justify-between gap-2">
        <span className="text-base font-semibold text-gray-900 truncate">{name}</span>
        <span
      ...
      {hasDomains && (
        <div className="mt-auto pt-1 border-t border-gray-100">
          <p className="text-xs text-gray-500 truncate">
            {domains![0]}
            {hasMultipleDomains && (
              <span className="ml-1 text-gray-400">+{domains!.length - 1} more</span>
            )}
          </p>
        </div>
      )}
